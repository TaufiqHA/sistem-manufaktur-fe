...
export const Warehouse: React.FC = () => {
  ...
  const processedItems = useMemo(() => {
    return items.map((item) => {
      const qcTasks = tasks.filter(
        (t) => t.itemId === item.id && t.step === "QC"
      );
      const totalPassedQC = qcTasks.reduce((acc, t) => acc + t.completedQty, 0);
      const pendingValidation = Math.max(
        0,
        totalPassedQC - (item.warehouseQty || 0)
      );
  ...
  const historyItemDetails = useMemo(() => {
    return (
      processedWarehouseItems.find((i) => i.id === historyItem) ||
      processedItems.find((i) => i.id === historyItem)
    );
  ...
  const warehouseList = processedWarehouseItems.filter(
    (p) =>
      p.status === "validated" &&
  ...
  const itemHistory = useMemo(() => {
    ...
    return logs.filter(
      (l) =>
        l.itemId === historyItem &&
        ((l.type === "OUTPUT" && l.step === "QC") ||
          l.type === "WAREHOUSE_ENTRY")
    );
  }, [logs, historyItem]);
  ...
  const handleValidate = async (id: string, qty: number) => {
    ...
    try {
      ...
      const warehouseItem = finishedGoodsWarehouses.find(
      ...
      if (warehouseItem) {
        ...
        if (response.success) {
          ...
          const warehouseResponse =
            await apiClient.getFinishedGoodsWarehouses();
          if (warehouseResponse.success && warehouseResponse.data) {
            const warehouseList = Array.isArray(warehouseResponse.data)
  ...
  const handleShipment = async () => {
    ...
    try {
      ...
      const newAvailableStock =
        shippingItem.availableStock - shippingItem.qtyToShip;

      // Send complete payload with all required fields
      const response = await apiClient.updateFinishedGoodsWarehouse(
        shippingItem.id,
        {
          project_id: shippingItem.projectId,
          item_name: shippingItem.name,
          total_produced: shippingItem.totalProduced,
      ...
      if (response.success) {
        // Create delivery order (surat jalan) automatically
        const project = projects.find((p) => p.id === shippingItem.projectId);
        const sjCode = `SJ-${new Date().getFullYear()}${String(
          new Date().getMonth() + 1
        ...
        const doResponse = await apiClient.createDeliveryOrder(
          deliveryOrderPayload
        );

        if (doResponse.success) {
          // Create delivery order item
          const doId = doResponse.data?.id || `do-${Date.now()}`;
    ...
    <div className="space-y-8 pb-20">
      <div>
        ...
        <p className="text-slate-500 font-bold mt-2 uppercase tracking-widest text-[10px]">
          Validasi Hasil Produksi QC &rarr; Stok Gudang
        </p>
      </div>
            ...
            <table className="w-full text-sm text-left min-w-[700px]">
              ...
              <tbody className="divide-y divide-slate-100 font-bold">
                {validationList.map((item) => {
                  ...
                  const qtyToValidate = isApiItem
                    ? item.availableStock
                    : item.pendingValidation;
                  const totalQty = isApiItem
                    ? item.totalProduced
                    : item.totalPassedQC;
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-8 py-5 text-blue-600 text-[10px] uppercase">
                        {item.projectName}
                      </td>
                ...
                {validationList.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="py-20 text-center text-slate-300 font-black italic uppercase text-xs"
                    >
                      Belum ada barang baru dari QC.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="relative w-64">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl font-bold text-xs"
              value={searchTerm}
                ...
                {warehouseList.map((it) => (
                  <tr key={it.id} className="hover:bg-slate-50/50 group">
                    ...
                    <td className="px-8 py-5 text-right flex gap-3 items-center justify-end">
                      <button
                        onClick={() => setHistoryItem(it.id)}
                        className="p-3 bg-slate-50 text-slate-400 hover:text-blue-600 rounded-2xl transition-all border border-slate-100 flex items-center gap-2 text-[10px] font-black uppercase"
                      >
                ...
                {warehouseList.length === 0 && !isLoading && (
                  <tr>
                    <td
                      ...
                      className="py-20 text-center text-slate-300 font-black italic uppercase text-xs"
                    >
                      Belum ada barang di gudang jadi.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto custom-scrollbar">
              {itemHistory.length > 0 ? (
                itemHistory.map((log) => (
                  <div
                    key={log.id}
                    className="bg-slate-50 p-6 rounded-[28px] border border-slate-100 flex justify-between items-center"
                  >
                    <div className="flex gap-4 items-center">
                      <div
                        className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${
                          log.type === "WAREHOUSE_ENTRY"
      ...
      {editingItem && (
        <div className="fixed inset-0 bg-slate-900/80 z-[400] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95">
            <div className="bg-amber-100 p-5 rounded-full text-amber-600 inline-block mb-6">
              <WhIcon size={40} />
            </div>
            <h3 className="text-xl font-black text-slate-900 uppercase">
              Validasi Qty Masuk
            </h3>
            <p className="text-xs text-slate-400 font-bold mt-1 uppercase tracking-widest mb-8">
              {editingItem.name}
            </p>
            <div className="space-y-6">
              <input
                type="number"
                disabled={isSaving}
                className="w-full p-6 bg-slate-50 border-2 border-slate-200 rounded-[28px] text-4xl font-black text-center outline-none focus:border-blue-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <div className="flex flex-col gap-3">
                <button
                  onClick={() =>
                    handleValidate(editingItem.id, editingItem.qty)
                  }
                  disabled={isSaving}
                  className="w-full py-5 bg-slate-900 text-white rounded-[24px] font-black uppercase text-sm tracking-widest shadow-xl flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Save size={18} />{" "}
                  {isSaving ? "MENYIMPAN..." : "SIMPAN KE GUDANG"}
                </button>
                <button
                  onClick={() => setEditingItem(null)}
                  disabled={isSaving}
                  className="text-slate-400 font-black text-[10px] uppercase tracking-widest py-2 disabled:opacity-50"
                >
                  Batalkan
                </button>
              </div>
      ...
      {shippingItem && (
        <div className="fixed inset-0 bg-slate-900/80 z-[400] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95">
            <div className="bg-emerald-100 p-5 rounded-full text-emerald-600 inline-block mb-6">
              <Package size={40} />
            </div>
            <h3 className="text-xl font-black text-slate-900 uppercase">
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-center gap-4">
                  <button
                    onClick={() =>
                      setShippingItem({
                        ...shippingItem,
                        qtyToShip: Math.max(0, shippingItem.qtyToShip - 1),
                      })
                    }
                    className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 font-black text-lg hover:bg-slate-200 transition-all"
                  >
                    -
                  </button>
                  <input
                    type="number"
                    className="w-24 text-center text-5xl font-black outline-none border-none bg-transparent"
                    value={shippingItem.qtyToShip}
                    onChange={(e) =>
                      setShippingItem({
                        ...shippingItem,
                        qtyToShip: Math.min(
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  disabled={isSaving || shippingItem.qtyToShip <= 0}
                  className="w-full py-5 bg-emerald-600 text-white rounded-[24px] font-black uppercase text-sm tracking-widest shadow-xl flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Package size={18} />{" "}
                  {isSaving ? "MENYIMPAN..." : "CATAT PENGIRIMAN"}
                </button>
                <button
