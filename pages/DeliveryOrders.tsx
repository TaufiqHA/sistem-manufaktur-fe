                   ...
                   <div className="p-8 bg-slate-50 border-r">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                         {projects.filter(p => p.status === 'IN_PROGRESS').map(p => {
                           ...
                           return (
                             <button 
                               key={p.id} disabled={isAdded} onClick={() => addProjectToSJ(p)}
                      ...
                      <table className="w-full text-left text-xs min-w-[800px]">
                         <thead className="bg-slate-50 text-slate-400 font-black uppercase tracking-widest border-b">
                            <tr>
                               ...
                               <th className="px-8 py-5 text-center">Kebutuhan Total</th>
                         ...
                         <tbody className="divide-y divide-slate-100 font-bold">
                            {sjData.items.map((item, idx) => {
                               ...
                               const itInStore = items.find(i => i.id === item.itemId);
                               const project = projects.find(p => p.id === item.projectId);
                               const totalRequired = (project?.qtyPerUnit || 0) * (itInStore?.qtySet || 0);
                               ...
                               return (
                                 <tr key={idx} className={`hover:bg-slate-50/50 ${isStockShort ? 'bg-red-50/30' : ''}`}>
                                   ...
                                   <td className="px-8 py-5 text-center text-slate-400 font-black">{totalRequired} {item.unit}</td>
